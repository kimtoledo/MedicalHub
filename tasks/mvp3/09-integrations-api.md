...
- **Real outbound webhook delivery (this update):** `appointment.created` (public booking) and `invoice.paid` (both manual clinic payment and online-payment webhook success) now dispatch to every active, subscribed clinic webhook. Deliveries are HMAC-SHA256 signed (`x-dentra-webhook-signature`), sent with a 5s timeout, and retried with exponential backoff (capped at 60 min) for up to 5 attempts before being marked permanently failed. `appointment.updated` is not wired yet — only creation and invoice-paid fire today.
...
- Remaining: provider credential connectors, iCal/Google Calendar export, accounting formats, `appointment.updated`/refund events, and read-write partner resources.
