...
- **Real outbound webhook delivery (this update):** `appointment.created` (public booking), `appointment.updated` (clinic-staff status changes and kiosk self-check-in), and `invoice.paid` (both manual clinic payment and online-payment webhook success) now dispatch to every active, subscribed clinic webhook. Deliveries are HMAC-SHA256 signed (`x-dentra-webhook-signature`), sent with a 5s timeout, and retried with exponential backoff (capped at 60 min) for up to 5 attempts before being marked permanently failed. Refund events are not wired yet.
...
- Remaining: provider credential connectors, iCal/Google Calendar export, accounting formats, refund events, and read-write partner resources.
