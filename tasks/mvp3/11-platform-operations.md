...
- Investigated per-tenant maintenance mode: `clinics.status` (`trial`/`active`/`suspended`/`archived`) exists but isn't a real enforcement gate today — `suspended` is only a Super Admin console badge plus a couple of one-off checks in `remote-consults-service.ts` and `clinic-settings-service.ts`; there's no single chokepoint (`hasClinicAccess`/`requireClinicFeature` don't read `status` at all, and are called individually across ~29 route files), and no clinic-facing blocking screen exists on the web side. This is the same shape of problem as the scoped support-access item above: mechanically small once scoped, but blocked on real product decisions — is maintenance a new `clinics.status` value or an orthogonal flag, does it block writes-only or everything, does Super Admin/support still get through, and which route families are actually in scope (enforcement today is already inconsistent per-route). Not guessing this.
- Remaining: retention/anonymization jobs, actual deletion/offboarding execution, restore drills, alerting, maintenance mode, and the scoped support enforcement above — all four blocked on the access-control/lifecycle-policy decisions named above.
