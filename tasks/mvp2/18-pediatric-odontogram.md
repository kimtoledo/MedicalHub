...
- Every condition/procedure code (fillings, extraction, root canal, implant, crown, bridge, scaling, bleaching, etc.) has a distinct icon marker on the tooth, not just a same-colored dot.
...
6. **Condition/procedure icons** — ✅ A `lucide-react` icon distinguishes each condition/procedure code on the tooth marker.
