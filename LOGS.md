...
- Added the Super Admin UI: a "Capacity limits" editor section on the Packages page, a new Capacity tab on the clinic detail page (effective usage-vs-limit display + `LimitOverrideManager` add/remove UI), and negotiated-price/billing-note fields plus warning surfacing on the change-package dialog
- Rewrote the clinic-facing subscription settings page (previously minified/untyped with no capacity visibility) to show live seat usage and a request form that supports upgrade/downgrade/add-extra-seats, the last of which submits a structured absolute seat target
- Verified against the real local dev database that capacity resolution and enforcement behave correctly on live data, and live-verified all three new UI surfaces in an actual browser (Super Admin packages editor, clinic detail Capacity tab, clinic-facing subscription page) with no console errors; 537 passing API tests + 18 passing web tests (10 new), repository-wide TypeScript checks, and clean production builds
