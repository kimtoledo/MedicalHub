...
const requestBody = z.object({ type: z.enum(['upgrade', 'downgrade', 'addon']), requestedPackageId: postgresUuidSchema.optional(), requestedMetric: z.enum(capacityMetricValues).optional(), requestedLimit: z.number().int().min(0).max(500).nullable().optional(), reason: z.string().trim().min(3).max(1000) }).strict();
...
export async function registerSubscriptionOperationRoutes(app: FastifyInstance, options: { auth: AuthServices; entitlements: EntitlementService; operations: SubscriptionOperationsService; adminSettings: AdminClinicSettingsService }) {
  ...
  app.get('/v1/clinic/:clinicId/packages', async (request, reply) => { const params = clinicParams.safeParse(request.params); if (!params.success) return reply.status(400).send({ success: false, error: { code: 'VALIDATION_ERROR', message: 'Invalid clinic identifier' } }); const auth = await resolveRequestAuthorization(request, options.auth); if (!auth) return reply.status(401).send({ success: false, error: { code: 'UNAUTHENTICATED', message: 'A valid session is required' } }); if (!hasClinicAccess(auth, params.data.clinicId)) return reply.status(403).send({ success: false, error: { code: 'FORBIDDEN', message: 'Clinic access is required' } }); return reply.send({ success: true, data: await options.operations.listAvailablePackages() }); });
