...
export function createPublicBookingService(database: DB, notifications?: NotificationService, integrations?: IntegrationService): PublicBookingService {
  ...
  return {
    ...
    book: async (input, request) => {
      ...
      const confirmationNumber = `DNT-${input.date.replaceAll('-', '')}-${created.id.slice(0, 8).toUpperCase()}`;
      const dentistName = `Dr. ${selected.firstName} ${selected.lastName}`;
      const notification = notifications && input.patientEmail
        ? await notifications.enqueue(transaction as unknown as DB, bookingConfirmationNotification({ patientName: `${input.patientFirstName} ${input.patientLastName}`, confirmationNumber, dentistName, serviceName: context.serviceName, endsAt: requested.endsAt, clinicId: context.clinicId, patientEmail: input.patientEmail, appointmentId: created.id, clinicName: context.clinicName, branchName: context.branchName, startsAt: requested.startsAt, dedupeKey: `booking-confirmation:${created.id}` }))
        : null;
      ...
      const reminder = notifications && input.patientEmail && remindsInMs > 0
        ...
        : null;
      return { appointmentId: created.id, clinicId: context.clinicId, notificationId: notification?.id ?? null, reminderId: reminder?.id ?? null, reminderDelayMs: remindsInMs, confirmationNumber, clinicName: context.clinicName, branchName: context.branchName, serviceName: context.serviceName, dentistName, startsAt: requested.startsAt, endsAt: requested.endsAt, status: 'pending' as const };
      });
