...
export type ClinicSettings = ClinicProfileInput & { id: string; name: string; slug: string; publicationStatus: string; logoUrl: string | null; coverUrl: string | null; logoUpdatedAt: Date | null; coverUpdatedAt: Date | null; branches: Array<{ id: string; name: string; hours: BranchHoursRowInput[] } & BranchLocationInput>; gallery?: Array<{ id: string; imageUrl: string; altText: string; caption: string | null; sortOrder: string; isPublished: boolean }> };
...
export function createClinicSettingsService(database: DB): ClinicSettingsService { return {
  get: async (clinicId) => {
    const [clinic] = await database.select({ id: clinics.id, name: clinics.name, slug: clinics.slug, publicationStatus: clinics.publicationStatus, heroText: clinics.heroText, description: clinics.description, email: clinics.email, phone: clinics.phone, website: clinics.website, address: clinics.address, city: clinics.city, province: clinics.province, mapUrl: clinics.mapUrl, facebookUrl: clinics.facebookUrl, instagramUrl: clinics.instagramUrl, themePreset: clinics.themePreset, brandAccent: clinics.brandAccent, coverMode: clinics.coverMode, logoUrl: clinics.logoUrl, coverUrl: clinics.coverUrl, logoUpdatedAt: clinics.logoUpdatedAt, coverUpdatedAt: clinics.coverUpdatedAt, showGallery: clinics.showGallery, showTeam: clinics.showTeam, showServices: clinics.showServices, seoTitle: clinics.seoTitle, seoDescription: clinics.seoDescription, ogImageUrl: clinics.ogImageUrl }).from(clinics).where(and(eq(clinics.id, clinicId), isNull(clinics.deletedAt))).limit(1);
