...
const service = (): EntitlementService => ({ resolve: vi.fn(async (id) => ({ clinic: { id, name: 'Smile Bright', status: 'active' }, subscription: { id: 'sub', status: 'active', package: { id: 'pkg', name: 'Professional', slug: 'professional' } }, entitlements: [{ featureKey: FeatureKey.APPOINTMENTS_MANAGE, isEnabled: true, source: 'package' as const, expiresAt: null }] })) });
