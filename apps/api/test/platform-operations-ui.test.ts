...
describe('platform operations console API', () => {
  ...
  it('lets a Super Admin mark an export request ready', async () => {
    const service = operations({ markExport: vi.fn(async () => ({ id: REQUEST_ID, status: 'ready' as const })) });
    await setup(adminContext(), service);
