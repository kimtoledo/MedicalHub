    ...
    <section id="clinics" className="py-20 px-4 sm:px-6 bg-white">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-8 relative">
          {/* Connector lines */}
          <div className="hidden md:block absolute top-12 left-1/3 right-1/3 h-0.5 bg-gradient-to-r from-teal-200 to-cyan-200" />
          ...
          {steps.map((s, i) => (
            <div key={s.step} className="flex flex-col items-center text-center relative">
              ...
              <h3 className="font-bold text-teal-900 text-xl mb-3">{s.title}</h3>
              <p className="text-teal-600 text-sm leading-relaxed">{s.desc}</p>
            </div>
          ))}
        </div>
      </div>
