...
export default function DentistScheduleEditor({ affiliations }: { affiliations: Array<{ branchId: string; clinicName: string; branchName: string }> }) {
  ...
  async function addTimeOff(event: FormEvent<HTMLFormElement>) {
    ...
    form.reset(); await loadTimeOff(); setMessage("Time off added."); setBusy(null);
  }
  ...
  const visibleTimeOff = hidePast ? timeOff.filter((entry) => entry.endDate >= todayKey) : timeOff;
  const pagedTimeOff = visibleTimeOff.slice(0, visibleCount);
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
    ...
    <div className="mt-6 border-t border-slate-100 pt-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <label className="flex items-center gap-2 text-xs font-semibold text-slate-600"><input type="checkbox" checked={hidePast} onChange={(event) => { setHidePast(event.target.checked); setVisibleCount(PAGE_SIZE); }} className="h-3.5 w-3.5 rounded border-slate-300" />Hide past dates</label>
      </div>
      <div className="mt-3 divide-y divide-slate-100 rounded-xl border border-slate-100">{visibleTimeOff.length === 0 && <p className="p-3 text-sm text-slate-400">{timeOff.length === 0 ? 'No upcoming time off.' : 'No upcoming time off. Uncheck "Hide past dates" to see past entries.'}</p>}{pagedTimeOff.map((entry) => <div key={entry.id} className="flex items-center justify-between gap-2 p-3"><div><p className="text-sm font-semibold text-slate-800">{entry.startDate} – {entry.endDate}</p>{entry.reason && <p className="text-xs text-slate-500">{entry.reason}</p>}</div><button type="button" disabled={busy === entry.id} onClick={() => void removeTimeOff(entry.id)} className="text-red-500 hover:text-red-700" aria-label="Remove time off"><Trash2 size={15} /></button></div>)}</div>
      {visibleTimeOff.length > visibleCount && <button type="button" onClick={() => setVisibleCount((c) => c + PAGE_SIZE)} className="mt-3 inline-flex h-9 w-full items-center justify-center rounded-xl border border-violet-200 text-xs font-semibold text-violet-700 hover:bg-violet-50">Load more ({visibleTimeOff.length - visibleCount} remaining)</button>}
