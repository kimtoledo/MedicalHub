  ...
  return <>
    ...
    <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
      {items.map((item) => <article key={item.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-4 text-sm"><span className="inline-flex items-center gap-2 text-slate-600"><Check size={15} className="text-emerald-600" />{item.featureKeys.length} features</span><span className="inline-flex items-center gap-2 text-slate-600"><Building2 size={15} className="text-violet-500" />{item.activeClinicCount} clinics</span></div>
      </article>)}
    </div>
