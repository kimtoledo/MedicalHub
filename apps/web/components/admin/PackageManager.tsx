...
import { AlertCircle, Building2, Check, Loader2, Pencil, Plus, Save, X } from 'lucide-react';
import type { AdminPackageItem } from '@/lib/admin-packages';

type Props = { items: AdminPackageItem[]; featureCatalog: string[] };
type Draft = { id?: string; name: string; slug: string; description: string; priceDisplay: string; isActive: boolean; featureKeys: string[] };
const emptyDraft: Draft = { name: '', slug: '', description: '', priceDisplay: 'Contact us', isActive: true, featureKeys: [] };
...
export default function PackageManager({ items, featureCatalog }: Props) {
  ...
  function edit(item?: AdminPackageItem) {
    ...
    setDraft(item ? { id: item.id, name: item.name, slug: item.slug, description: item.description ?? '', priceDisplay: item.priceDisplay, isActive: item.isActive, featureKeys: [...item.featureKeys] } : { ...emptyDraft, featureKeys: [] });
  }
  ...
  async function save(event: FormEvent) {
    ...
    const response = await fetch(draft.id ? `/api/admin/packages/${draft.id}` : '/api/admin/packages', {
      ...
      body: JSON.stringify({ name: draft.name, slug: draft.slug, description: draft.description, priceDisplay: draft.priceDisplay, isActive: draft.isActive, featureKeys: draft.featureKeys }),
    }).catch(() => null);
    if (!response?.ok) {
  ...
  return <>
    ...
    <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
      {items.map((item) => <article key={item.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-4 text-sm"><span className="inline-flex items-center gap-1.5 text-slate-600"><Check size={15} className="text-emerald-600" />{item.featureKeys.length} features</span><span className="inline-flex items-center gap-1.5 text-slate-600"><Building2 size={15} className="text-violet-500" />{item.activeClinicCount} clinics</span></div>
      </article>)}
    </div>
