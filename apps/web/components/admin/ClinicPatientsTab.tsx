...
export default function ClinicPatientsTab({ clinicId }: { clinicId: string }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    void fetch(`/api/admin/clinics/${clinicId}/patients?page=${page}&pageSize=${PAGE_SIZE}`, {
      cache: 'no-store',
  ...
  if (!result || result.items.length === 0) {
    return (
      <div className="flex flex-col items-center gap-2 py-12 text-center">
        ...
        <p className="text-sm font-medium text-slate-500">No patients registered at this clinic yet</p>
      </div>
    );
  }
