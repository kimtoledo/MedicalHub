...
async function get<T>(path: string, clinicId: string): Promise<T> { const url = getBackendUrl(path); url.searchParams.set('clinicId', clinicId); const response = await fetch(url, { headers: { cookie: (await cookies()).toString() }, cache: 'no-store' }); if (!response.ok) throw new Error(`Treatment API failed with ${response.status}`); return ((await response.json()) as { data: T }).data; }
