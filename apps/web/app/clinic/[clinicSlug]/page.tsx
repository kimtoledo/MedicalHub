...
function clinicJsonLd(clinic: PublicClinicDetail, reviewSummary: { averageRating: number; total: number }) {
  ...
  return {
    ...
    image: brandingImageUrl(clinic.id, 'logo', clinic.logoUpdatedAt, clinic.logoUrl) ?? undefined,
...
export default async function ClinicMicrosite(props: Props) {
  const params = await props.params;
  ...
  const coverSrc = clinic.coverMode !== 'gradient' ? brandingImageUrl(clinic.id, 'cover', clinic.coverUpdatedAt, clinic.coverUrl) : null;
  const gradientClass = themeGradientClass(clinic.themePreset);
  const accent = clinic.brandAccent && /^#[0-9a-fA-F]{6}$/.test(clinic.brandAccent) ? clinic.brandAccent : '#7C3AED';
  return (
    <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(clinicJsonLd(clinic, reviewSummary)).replace(/</g, '\\u003c') }} /><main className="bg-[#f8f7ff]" style={{ '--brand-accent': accent } as React.CSSProperties}><section className={`relative overflow-hidden bg-gradient-to-br ${gradientClass} px-4 py-16 text-white sm:px-6 sm:py-24`}><div className="absolute inset-0 opacity-10" style={coverSrc ? { backgroundImage: `url(${coverSrc})`, backgroundSize: 'cover', backgroundPosition: 'center' } : undefined} /><div className="relative mx-auto flex max-w-6xl flex-col items-start gap-6 sm:flex-row sm:items-center"><div className="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-white/15 text-2xl font-extrabold ring-1 ring-white/25">{logoSrc ? <img src={logoSrc} alt={`${clinic.name} logo`} className="h-full w-full object-cover" /> : clinic.name.slice(0, 2).toUpperCase()}</div><div><p className="text-sm font-semibold uppercase tracking-wider text-violet-200">Dentra.ph clinic</p><h1 className="mt-2 text-4xl font-extrabold sm:text-5xl">{clinic.name}</h1><p className="mt-4 max-w-2xl text-lg leading-8 text-violet-100">{clinic.heroText ?? clinic.description ?? 'Professional dental care for healthier smiles.'}</p><Link href={`/clinic/${clinic.slug}/appointment`} className="mt-7 inline-flex items-center gap-2 rounded-full bg-white px-6 py-3 font-bold text-[var(--brand-accent)] shadow-lg hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-white">Book Appointment <ArrowRight size={18} /></Link></div></div></section>
    ...
    <section><h2 className="text-2xl font-bold text-slate-900">Meet the dentists</h2>{clinic.dentists.length ? <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{clinic.dentists.map((dentist) => <article key={dentist.id} className="rounded-2xl border border-violet-100 bg-white p-5"><div className="flex h-12 w-12 items-center justify-center rounded-xl bg-violet-100 font-bold text-violet-700">{dentist.firstName[0]}{dentist.lastName[0]}</div><h3 className="mt-3 font-bold text-slate-900"><Link href={`/dentists/${dentist.slug}`} className="hover:text-violet-700">Dr. {dentist.firstName} {dentist.lastName}</Link></h3><p className="mt-1 text-sm text-violet-600">{dentist.specialty ?? 'General Dentistry'}</p><p className="mt-2 text-xs text-slate-500">{dentist.branches.join(' · ')}</p><Link href={`/clinic/${clinic.slug}/appointment?dentist=${encodeURIComponent(dentist.id)}`} className="mt-4 inline-flex text-sm font-semibold text-[var(--brand-accent)] hover:opacity-80">Book with this dentist →</Link></article>)}</div> : <p className="mt-4 text-slate-500">Contact the clinic for its current dentist roster.</p>}</section>
    <PublicReviews clinicId={clinic.id} />
    <section className="rounded-3xl bg-[var(--brand-accent)] px-6 py-10 text-center text-white"><h2 className="text-3xl font-extrabold">Ready to book your visit?</h2><p className="mt-3 text-violet-100">Choose a branch, service, dentist, and available schedule online.</p><Link href={`/clinic/${clinic.slug}/appointment`} className="mt-6 inline-flex items-center gap-2 rounded-full bg-white px-6 py-3 font-bold text-[var(--brand-accent)]">Book Appointment <ArrowRight size={18} /></Link></section>
    </section></main><footer className="border-t border-violet-100 bg-white py-6 text-center text-xs text-slate-400">Powered by <a href="https://dentra.ph" className="font-semibold text-violet-500 hover:text-violet-700">Dentra.ph</a></footer></>
  );
}
