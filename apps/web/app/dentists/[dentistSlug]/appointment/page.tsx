...
type Props = { params: Promise<{ dentistSlug: string }>; searchParams: Promise<{ clinic?: string; branch?: string }> };
export async function generateMetadata(props: Props): Promise<Metadata> {
  const params = await props.params;
  const dentist = await getPublicDentist(params.dentistSlug).catch(() => null);return dentist ? { title: `Book with Dr. ${dentist.firstName} ${dentist.lastName}`, description: `Request a dental appointment with Dr. ${dentist.firstName} ${dentist.lastName}.` } : { title: 'Dentist Not Found', robots: { index: false } };
}
export default async function DentistAppointmentPage(props: Props) {
