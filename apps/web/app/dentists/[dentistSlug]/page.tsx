...
type Props = { params: Promise<{ dentistSlug: string }> };
...
export default async function DentistProfile(props: Props) {
  const params = await props.params;
  const dentist = await getPublicDentist(params.dentistSlug).catch(() => null);
  if (!dentist) notFound();
  ...
  return (
    <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(dentistJsonLd(dentist, reviewSummary)).replace(/</g, '\\u003c') }} /><Navbar /><main className="min-h-screen bg-[#f8f7ff]">
      ...
      <div className="mx-auto max-w-5xl space-y-10 px-4 py-12 sm:px-6"><section className="rounded-2xl border border-violet-100 bg-white p-6 shadow-sm"><h2 className="text-2xl font-bold text-slate-900">Professional biography</h2><p className="mt-4 whitespace-pre-line leading-7 text-slate-600">{dentist.bio ?? 'Contact an affiliated clinic to learn more about this dentist.'}</p>{serviceTags.length > 0 && <div className="mt-6 flex flex-wrap gap-2">{serviceTags.map((service) => <span key={service} className="rounded-full bg-violet-50 px-3 py-2 text-xs font-medium text-violet-700">{service}</span>)}</div>}</section>
